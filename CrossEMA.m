function [Finalny_nahradovy_pomer, AFS, w_SAP] = CrossEMA(Doba_sporenia_mesiace, Realne_logaritmicke_mesacne_vynosy_SAP_500, Realne_logaritmicke_mesacne_vynosy_dlhopis, Realne_hodnoty_pociatocnej_investicie_SAP_500, Realne_hodnoty_SeLFIES, Cieleny_realny_dochodkovy_prijem, Prispevky) % Funkcia na simuláciu dôchodkového sporenia pomocou stratégie CrossEMA
% Vykoná historickú simulácia dôchodkovej sporivej stratégie CrossEMA
% Vypočíta finálny náhradový pomer a vráti vývoj AFS a podielu investovaných prostiedkov do akciového indexu S&P 500 pre dôchodkovú investičnú stratégiu CrossEMA
...
% Výstupy:
% Finálny náhradový pomer - výsledný finálny náhradový pomer sporiteľa dosiahnutý dochodkovou investičnou stratégiou CrossEMA
% AFS - jednotlivé hodnoty aktuálneho stavu financovania (AFS) pre stratégiu CrossEMA
% w_SAP - jednotlivé hodnoty podielu investovaných prostiedkov do akciového indexu S&P 500 pre stratégiu CrossEMA

% Parametre dôchodkovej investičnej stratégie CrossEMA:
p_s = 2; % 2 mesačné krátke obdobie výpočtu exponenciálneho kĺzavého priemeru
p_l = 6; % 6 mesačné dlhé obdobie výpočtu exponenciálneho kĺzavého priemeru

% Historická simulácia:
% Inicializácia vektorov:
EMA_p_s = zeros(1, Doba_sporenia_mesiace); % Inicializácia vektora hodnôt EMA pre 2 mesačné krátke obdobie výpočtu exponenciálneho kĺzavého priemeru v dôchodkovej investičnej stratégii CrossEMA
EMA_p_l = zeros(1, Doba_sporenia_mesiace); % Inicializácia vektora hodnôt EMA pre 6 mesačné dlhé obdobie výpočtu exponenciálneho kĺzavého priemeru v dôchodkovej investičnej stratégii CrossEMA
...
for Mesiac_sporenia = 2:Doba_sporenia_mesiace % Pre každý mesiac sporenia
    EMA_p_s(Mesiac_sporenia) = (Realne_hodnoty_pociatocnej_investicie_SAP_500(Mesiac_sporenia) * (2 / (p_s + 1))) + (EMA_p_s(Mesiac_sporenia - 1) * (1 - (2 / (p_s + 1)))); % Hodnota EMA pre 2 mesačné krátke obdobie výpočtu exponenciálneho kĺzavého priemeru v dôchodkovej investičnej stratégii CrossEMA
    EMA_p_l(Mesiac_sporenia) = (Realne_hodnoty_pociatocnej_investicie_SAP_500(Mesiac_sporenia) * (2 / (p_l + 1))) + (EMA_p_l(Mesiac_sporenia - 1) * (1 - (2 / (p_l + 1)))); % Hodnota EMA pre 6 mesačné dlhé obdobie výpočtu exponenciálneho kĺzavého priemeru v dôchodkovej investičnej stratégii CrossEMA
    ...
    if EMA_p_s(Mesiac_sporenia) > EMA_p_l(Mesiac_sporenia) % Ak platí rozhodovacie kritérium v dôchodkovej investičnej stratégii CrossEMA
       w_SAP(Mesiac_sporenia) = 1; % Alokácia sporiteľových prostiedkov do akciového indexu S&P 500
    else % Ak neplatí rozhodovacie kritérium v dôchodkovej investičnej stratégii CrossEMA
